...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        q_max = self.computeValueFromQValues(state)

        for action in action_values.keys():
            if q_max == action_values[action]:
                action_max.append(action)

        return random.choice(action_max)
    ...
    def update(self, state, action, nextState, reward):
        ...
        utility = reward + self.discount * self.getValue(nextState)
        self.q_values[(state, action)] = (1 - self.alpha) * old_qvalue + self.alpha * utility
