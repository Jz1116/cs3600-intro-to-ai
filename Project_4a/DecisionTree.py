...
class Tree:
  ...
  def classify(self, classificationData):
    ...
    if self.root.isleaf:
        ...
        child = self.root.children[classificationData[self.root.attr]]
        new_tree = Tree(child)
        return new_tree.classify(classificationData)
...
def getAttributeCounts(examples,attrName,attrValues,className):
    ...
    for attr in attrValues:
        attr_example = []
        for example in examples:
            if example[attrName] == attr:
                attr_example.append(example)
        attributeCounts[attr] = getClassCounts(attr_example, className)
...
def setEntropy(classCounts):
    ...
    for count in classCounts:
        ratio = float(count) / sum(classCounts)
        if ratio != 0:
...
def remainder(examples,attrName,attrValues,className):
    ...
    """
    #YOUR CODE HERE
    attribute_count = getAttributeCounts(examples, attrName, attrValues, className)
    total = len(examples)

    remainder = 0.0
    for attr in attribute_count:
        attr_class_total = sum(attribute_count[attr].values())
        attr_ratio = float(attr_class_total) / total
        remainder_part = 0.0

        for count in attribute_count[attr].values():
            class_ratio = float(count) / attr_class_total
            if class_ratio != 0:
                remainder_part -= class_ratio * log(class_ratio, 2)

        remainder_part *= attr_ratio
...
def infoGain(examples,attrName,attrValues,className):
    ...
    H = setEntropy(class_count.values())

    remainder_ = remainder(examples, attrName, attrValues, className)

    info_gain = H - remainder_
...
def giniGain(examples,attrName,attrValues,className):
    ...
    """
    #YOUR CODE HERE
    attr_count = getAttributeCounts(examples, attrName, attrValues, className)

    total = len(examples)

    gini = 0.0
    for attr in attr_count:
        attr_class_total = sum(attr_count[attr].values())
        attr_ratio = float(attr_class_total) / total

        gini_part = giniIndex(attr_count[attr].values())
        gini_part *= attr_ratio
...
def makeSubtrees(remainingAttributes,examples,attributeValues,className,defaultLabel,setScoreFunc,gainFunc):
    ...
    for attr_name in remainingAttributes:
        local_info_gain = gainFunc(examples, attr_name, attributeValues[attr_name], className)
        if not_assigned:
    ...
    for attr_value in attributeValues[best_attr]:
        ...
        child = makeSubtrees(remaining_attributes, subset_examples, attributeValues,className,getMostCommonClass(examples, className),setScoreFunc,gainFunc)
...
def makePrunedSubtrees(remainingAttributes,examples,attributeValues,className,defaultLabel,setScoreFunc,gainFunc,q):
    ...
    # chi-square
    dict_1 = getAttributeCounts(examples, best_attr, attributeValues[best_attr], className)
    dict_2 = {}
    ...
    for attr_value in attributeValues[best_attr]:
        subset_examples = getPertinentExamples(examples, best_attr, attr_value)
        child = makePrunedSubtrees(remaining_attributes, subset_examples, attributeValues,className,getMostCommonClass(examples, className),setScoreFunc,gainFunc, q)
        root.children[attr_value] = child
