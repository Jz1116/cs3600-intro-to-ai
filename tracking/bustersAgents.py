    ...
    def chooseAction(self, gameState):
        ...
        for positionDistribution in livingGhostPositionDistributions:
            ...
            for position in positionDistribution.keys():
                if positionDistribution[position] > max_prob:
                    max_prob = positionDistribution[position]
                    max_prob_location = position
            livingGhostBestLocation.append(max_prob_location)

        min_distance = float("inf")
        min_ghost_position = None
        best_action = None
        for ghost_position in livingGhostBestLocation:
            distance = self.distancer.getDistance(pacmanPosition, ghost_position)
            if min_distance > distance:
                min_ghost_position = ghost_position
                min_distance = distance

        distance_in_between = float('inf')
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            distance = self.distancer.getDistance(successorPosition, min_ghost_position)
            if distance_in_between > distance:
                distance_in_between = distance
                best_action = action
