    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for ghost_position in self.legalPositions:
                trueDistance = util.manhattanDistance(ghost_position, pacmanPosition)
                allPossible[ghost_position] = self.beliefs[ghost_position] * emissionModel[trueDistance]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for index in range(self.numParticles):
            ...
            particles.append(self.legalPositions[position])
    ...
    def observe(self, observation, gameState):
        ...
        for particle in self.particles:
            ...
            particle_weights[particle] += emissionModel[distance]
        ...
        for i in range(self.numParticles):
            sample = util.sample(particle_weights)
            weighted_particle.append(sample)

        self.particles = weighted_particle
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            if oldPos not in distribution:
                distribution[oldPos] = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            sample = util.sample(distribution[oldPos])
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        "*** YOUR CODE HERE ***"
        permutations = []
        for product in itertools.product(self.legalPositions, repeat=self.numGhosts):
            permutations.append(product)

        util.FixedRandom().random.shuffle(permutations)
            ...
            for ghost_index in range(self.numGhosts):
                if noisyDistances[ghost_index] == None:
                    for particle_index in range(len(self.particles)):
                        self.particles[particle_index] = self.getParticleWithGhostInJail(self.particles[particle_index], ghost_index)
